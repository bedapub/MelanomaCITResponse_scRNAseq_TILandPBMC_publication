...
PBMC data is loaded. Comparison at celltype3 and celltype2 level, NR (adjuvant only) vs. R (TF only); 
change selection for NR (nadj) vs. R for non-adjuvant analysis. Cell numbers are used as weights. Patients P38, P12, P87, P67, P68  removed. 


```{r}

mymat <- readr::read_tsv('../analyzed/sw_besca2_immune_res2.tsv')
...
mymat <- mymat %>%
  ...
  mutate(cell_names = replace(cell_names, cell_names == "CD8-positive, cytotoxic T cell", "exhausted-like CD8-positive T cell")) %>%
  mutate(cell_names = replace(cell_names, cell_names == "MARCO-positive macrophage", "macrophage"))
    
mymat=mymat[mymat$RCat%in%c("TF","NR_adj"),]
...
Responders have overall lower cell numbers. 
...
avail <- unique(mymat_summary %>% select(cell_names, count) %>%   
                  group_by(cell_names) %>% 
                  summarise(count = mean(count))%>% dplyr::filter(count>30)  %>% .$cell_names)
...
Using an additional random effect for each sample did not change anything (Malgorzata 2017).
...
avai2 <- unique(mymat_hl_summary %>% select(sscell_group, count) %>%   
                  ...
                  summarise(count = mean(count))%>% dplyr::filter(count>30)  %>% .$sscell_group)
